...
const RefundMemberRecord:FC<IProps> = ({onOk,visible,data}) => {
    ...
    return <Modal
        ...
        title={<div style={{color:"#fff",fontWeight:550}}>{t("REFUND")}</div>}
        visible={visible}
        onCancel={ onCancel}
        onOk={onFinish}
        okText={t('SAVE')}
    ...
    >
        <Form form={form} className="email-new" initialValues={{status:1,subscription:[1]}}>
            ...
            <Form.Item name="remark" label={<span className="login-label">{t('REFUND_REASON')}</span>}>
                <Input/>
            </Form.Item>
        </Form>
    </Modal>
...
export default RefundMemberRecord;
